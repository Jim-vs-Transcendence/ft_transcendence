...
export class GameService {
	...
	public sendGameData(room: Room) {
		...
		if (room.leftPlayer.updateData.moveData.ballX >= this.canvasWidth - this.ballRadius * 2) {
			room.leftPlayer.updateData.leftScore++;
			this.resetGame(room);
		}
		if (!room.isEnd) {
			if (room.leftPlayer.updateData.moveData.ballY <= this.ballRadius) {
				room.leftPlayer.updateData.moveData.ballMoveY = false;
				room.rightPlayer.updateData.moveData.ballMoveY = false;
			}
			if (room.leftPlayer.updateData.moveData.ballY >= this.canvasHeight - this.ballRadius) {
			...
			if (room.leftPlayer.updateData.moveData.ballX - (this.ballRadius * 2) <= this.initLeftPaddleX && room.leftPlayer.updateData.moveData.ballX >= this.initLeftPaddleX - this.paddleWidth) {
				if (room.leftPlayer.updateData.moveData.ballY <= room.leftPlayer.updateData.moveData.leftPaddleY + this.paddleHeight && room.leftPlayer.updateData.moveData.ballY >= room.leftPlayer.updateData.moveData.leftPaddleY) {
					room.leftPlayer.updateData.moveData.ballX = this.initLeftPaddleX + this.ballRadius * 2;
					room.leftPlayer.updateData.moveData.ballMoveX = false;
					room.rightPlayer.updateData.moveData.ballX = this.initRightPaddleX - this.ballRadius * 2;
					room.rightPlayer.updateData.moveData.ballMoveX = true;

				}
			}

			if (room.leftPlayer.updateData.moveData.ballX - (this.ballRadius * 2) <= this.initRightPaddleX && room.leftPlayer.updateData.moveData.ballX >= this.initRightPaddleX - this.paddleWidth) {
				if (room.leftPlayer.updateData.moveData.ballY <= room.leftPlayer.updateData.moveData.rightPaddleY + this.paddleHeight && room.leftPlayer.updateData.moveData.ballY >= room.leftPlayer.updateData.moveData.rightPaddleY) {
					room.leftPlayer.updateData.moveData.ballX = this.initRightPaddleX - this.ballRadius * 2;
					room.leftPlayer.updateData.moveData.ballMoveX = true;
					room.rightPlayer.updateData.moveData.ballX = this.initLeftPaddleX + this.ballRadius * 2;
